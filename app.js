/* ==========================================================
   ...
   Changes from v8:
   - CRF 14-20 (was 16-22) — preserves much more detail
   - Preset 'medium' (was 'fast') — smarter compression
   - Lanczos scaling (was bilinear) — razor sharp downscale
   - Dynamic bitrate by duration — short videos get more bits
   - aq-mode=2 — better dark/detail areas (safe for WASM)
   - Processing takes longer but quality is dramatically better
   ========================================================== */
...
var CONFIG = {
    ...
    quality: {
        shortSide    : 720,
        baseCRF      : 16,         // was 20 — much higher quality base
        audioBitrate : '128k',
        audioRate    : 44100,
        audioChannels: 2,
        fps          : 30,
        preset       : 'medium',   // was 'fast' — better quality per bit
        profile      : 'high',
        level        : '4.0',
        keyint       : 30,
        targetMaxMB  : 8,
...
function initEls() {
    ...
    var missing = [];
    for (var key in els) { if (!els[key]) missing.push(key); }
    if (missing.length > 0) log('⚠️ Missing DOM: ' + missing.join(', '));
...
function isPiPSupported() {
    try { return !!(document.pictureInPictureEnabled && HTMLVideoElement.prototype.requestPictureInPicture); }
...
function drawPipFrame() {
    ...
    ctx.font = '36px serif'; ctx.textAlign = 'center'; ctx.fillText('🍳', w/2, 42);
    ctx.font = '600 14px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.8)';
    ctx.fillText(state.pipStatusText, w/2, 68);
    var bx=30,by=90,bw=w-60,bh=16,br=8;
    ctx.fillStyle = 'rgba(255,255,255,0.1)'; roundRect(ctx,bx,by,bw,bh,br); ctx.fill();
    if (pct > 0) {
        var fw = Math.max(bh, (bw*pct)/100);
        var gr = ctx.createLinearGradient(bx,0,bx+bw,0);
        gr.addColorStop(0,'#FF6B35'); gr.addColorStop(1,'#FF3366');
        ctx.fillStyle = gr; roundRect(ctx,bx,by,fw,bh,br); ctx.fill();
    }
    ctx.font = '800 28px sans-serif'; ctx.fillStyle = '#FFF';
...
async function startPiP() {
    ...
    try {
        ...
        state.pipProgress = 0; state.pipStatusText = 'Starting…'; drawPipFrame();
    ...
    } catch (err) { log('PiP failed: ' + err.message); cleanupPiP(); return false; }
}

function updatePiP(pct, txt) {
    state.pipProgress = pct; if (txt) state.pipStatusText = txt;
...
function cleanupPiP() {
    ...
    if(state.pipVideo){
        ...
        if(state.pipVideo.srcObject){try{state.pipVideo.srcObject.getTracks().forEach(function(t){t.stop();});}catch(e){}state.pipVideo.srcObject=null;}
...
function startSilentAudio() {
    try {
        ...
        var src=state.silentAudioCtx.createBufferSource();src.buffer=buf;src.loop=true;
        var g=state.silentAudioCtx.createGain();g.gain.value=0.001;
        src.connect(g);g.connect(state.silentAudioCtx.destination);src.start();
...
async function acquireWakeLock() {
    ...
    state.wakeLock.addEventListener('release',function(){if(state.processing)acquireWakeLock();});}catch(e){}
}
...
function sendNotification(title,body) {
    ...
    try{var n=new Notification(title,{body:body,tag:'crispy-status',vibrate:[200,100,200]});
    n.onclick=function(){window.focus();n.close();};}catch(e){}
}

function setupVisibilityHandler() {
    document.addEventListener('visibilitychange',function(){
        if(document.visibilityState==='hidden'){state.wasHidden=true;if(state.processing)acquireWakeLock();}
        else{if(state.wasHidden&&state.processing)showToast('Still crisping… 🍳','info',2000);
        if(state.wasHidden&&!state.processing&&state.outputUrl)showToast('Your video is ready! 🔥','success');
        state.wasHidden=false;}
    });
}
function setupFreezeHandler() {
    try{document.addEventListener('freeze',function(){});
    document.addEventListener('resume',function(){if(state.processing)showToast('Resumed ▶️','info',2000);});}catch(e){}
}

/* ======================== SCREENS ======================== */
function showScreen(id) {
    document.querySelectorAll('.screen').forEach(function(s){s.classList.remove('active','screen-enter');});
    var el=$(id); if(el)el.classList.add('active','screen-enter');
...
function fireConfetti() {
    ...
    var particles=[];
    for(var i=0;i<90;i++)particles.push({x:canvas.width/2+(Math.random()-0.5)*100,y:canvas.height*0.35,vx:(Math.random()-0.5)*18,vy:Math.random()*-20-8,size:Math.random()*8+4,color:colors[Math.floor(Math.random()*colors.length)],rotation:Math.random()*360,rotSpeed:(Math.random()-0.5)*12,gravity:0.4+Math.random()*0.2,opacity:1,shape:Math.random()>0.5?'circle':'rect'});
    var frame=0;
    function draw(){
        ctx.clearRect(0,0,canvas.width,canvas.height);
        particles.forEach(function(p){p.x+=p.vx;p.y+=p.vy;p.vy+=p.gravity;p.vx*=0.99;p.rotation+=p.rotSpeed;p.opacity=Math.max(0,1-frame/150);ctx.save();ctx.globalAlpha=p.opacity;ctx.translate(p.x,p.y);ctx.rotate(p.rotation*Math.PI/180);ctx.fillStyle=p.color;if(p.shape==='circle'){ctx.beginPath();ctx.arc(0,0,p.size/2,0,Math.PI*2);ctx.fill();}else ctx.fillRect(-p.size/2,-p.size/2,p.size,p.size);ctx.restore();});
        frame++;if(frame<150)requestAnimationFrame(draw);else ctx.clearRect(0,0,canvas.width,canvas.height);
    }
    requestAnimationFrame(draw);
}

function haptic(style){try{if(navigator.vibrate)navigator.vibrate({light:[12],medium:[25],heavy:[50],success:[15,50,15]}[style||'light']||[12]);}catch(e){}}

function showError(title,msg){
    try{var h=$('error-heading');if(h)h.textContent=title;
    if(els.errorMsg)els.errorMsg.textContent=msg;
    if(els.errorModal)els.errorModal.classList.remove('hidden');haptic('heavy');}
...
function handleFileSelect(file) {
    ...
    if(file.size>CONFIG.maxFileSize*1024*1024){showError('File too large','Please pick a video under '+CONFIG.maxFileSize+' MB.');return;}
    if(file.size<10000){showError('File too small','This file seems too small to be a video.');return;}
    ...
    els.trimVideo.onloadedmetadata=function(){
        ...
        log('Video:',{dur:state.duration.toFixed(1)+'s',dim:state.videoWidth+'×'+state.videoHeight});
...
function setButtonLoading(btn,loading){if(!btn)return;if(loading)btn.classList.add('loading');else btn.classList.remove('loading');}
...
function updateTrimUI() {
    var start=parseFloat(els.trimSlider.value),dur=state.duration,clipDur=Math.min(CONFIG.maxDuration,dur);
...
async function loadFFmpeg() {
    ...
    state.ffmpeg.on('progress',function(ev){var pct=Math.min(Math.round(ev.progress*100),100);if(pct>0)setProgress(pct);});
...
async function createWatermarkImage() {
    ...
    var hp=10,vp=8;
    canvas.width=tw+hp*2;canvas.height=wm.fontSize+vp*2;
    ...
    ctx.textBaseline='middle';ctx.fillText(wm.text,hp,canvas.height/2);
    return new Promise(function(resolve,reject){
        canvas.toBlob(function(blob){if(!blob){reject(new Error('WM fail'));return;}
...
/* ============================================================
   QUALITY ENGINE — The heart of the improvement
   ============================================================ */

/* Smart CRF: shorter videos get MUCH lower CRF (= higher quality)
   because the file stays small anyway */
function getSmartCRF(durationSec) {
    var base = CONFIG.quality.baseCRF;  // 16
    if (durationSec <= 5)  { log('CRF: ' + (base - 2) + ' (ultra short — max quality)');  return base - 2; }  // 14
    if (durationSec <= 10) { log('CRF: ' + base + ' (short — high quality)');              return base; }      // 16
    if (durationSec <= 20) { log('CRF: ' + (base + 2) + ' (medium — great quality)');     return base + 2; }  // 18
    log('CRF: ' + (base + 4) + ' (full 30s — good quality)');
    return base + 4;  // 20
}

/* Smart bitrate: shorter videos can handle MUCH higher bitrate
   because total file size stays under WhatsApp limits */
function getSmartBitrate(durationSec) {
    if (durationSec <= 5) {
        log('Bitrate: 5000k (ultra short — unlimited quality)');
        return { rate: '5000k', buf: '10000k' };
    }
    if (durationSec <= 10) {
        log('Bitrate: 4000k (short — very high)');
        return { rate: '4000k', buf: '8000k' };
    }
    if (durationSec <= 20) {
        log('Bitrate: 3000k (medium — high)');
        return { rate: '3000k', buf: '6000k' };
    }
    log('Bitrate: 2500k (full 30s — optimized)');
    return { rate: '2500k', buf: '5000k' };
}

/* Lanczos scaling — dramatically sharper than default bilinear */
function buildScaleFilter() {
    var target = CONFIG.quality.shortSide;
    if (state.isPortrait) {
        if (state.videoWidth <= target) {
            log('Scale: native (no upscale) + lanczos alignment');
            return 'scale=trunc(iw/2)*2:trunc(ih/2)*2:flags=lanczos';
        }
        log('Scale: portrait → ' + target + 'w (lanczos)');
        ...
        log('Scale: landscape → ' + target + 'h (lanczos)');
...
function buildFFmpegCommand(clipDuration, hasWatermark) {
    ...
    var br = getSmartBitrate(clipDuration);
    var scale = buildScaleFilter();
    var pad = CONFIG.watermark.padding;

    log('=== ENCODING CONFIG ===');
    log('Resolution: ' + q.shortSide + 'p | CRF: ' + crf + ' | Preset: ' + q.preset);
    log('Bitrate: ' + br.rate + ' max | Profile: ' + q.profile + ' | Scaler: lanczos');
    log('Duration: ' + clipDuration.toFixed(1) + 's | Watermark: ' + hasWatermark);
    ...
    cmd.push(
        '-c:v', 'libx264',
        '-crf', String(crf),
        '-preset', q.preset,           // medium — better quality per bit
        '-profile:v', q.profile,       // high — best compression efficiency
        '-level:v', q.level,
        '-maxrate', br.rate,           // dynamic by duration
        '-bufsize', br.buf,            // 2× maxrate
        '-g', String(q.keyint),
        '-keyint_min', String(q.keyint),
        '-r', String(q.fps),
        '-pix_fmt', 'yuv420p',
        '-x264-params', 'aq-mode=2',  // adaptive quantization — safe, ~5MB extra RAM
        '-c:a', 'aac',
        '-b:a', q.audioBitrate,
...
async function startProcessing() {
    ...
    if (isMobile() && isPiPSupported()) {
        try { pipStarted = await startPiP(); } catch(e) {}
    }
    if (!pipStarted && isMobile()) showToast('⚠️ Keep app open — high quality encoding in progress', 'info', 5000);
    ...
    try {
        ...
        log('File: ' + formatBytes(fileData.byteLength));
        ...
        // Encode
        updateStatus('Making it crispy (high quality)… 🍳');
        updatePiP(0, 'Encoding HD…');
        var clipDur = Math.min(CONFIG.maxDuration, state.duration);
        ...
        // Fallback without watermark if failed
        if (exitCode !== 0 && state.hasWatermark) {
            log('⚠️ Retrying without watermark…');
            updateStatus('Retrying… 🔄');
            state.hasWatermark = false;
            var fallback = buildFFmpegCommand(clipDur, false);
            exitCode = await state.ffmpeg.exec(fallback);
            log('Fallback exit: ' + exitCode);
        }

        // Fallback with simpler settings if still failed
        if (exitCode !== 0) {
            log('⚠️ Retrying with safe settings…');
            updateStatus('Trying safe mode… 🔧');
            var safeCmd = [
                '-y', '-ss', String(state.trimStart), '-i', 'input',
                '-t', String(clipDur),
            ...
            exitCode = await state.ffmpeg.exec(safeCmd);
            log('Safe mode exit: ' + exitCode);
        }
        ...
        var bitrate = ((state.outputSize * 8) / (clipDur * 1000)).toFixed(0);
        log('✅ DONE in ' + elapsed + 's');
        log('   Size: ' + sizeMB.toFixed(2) + 'MB | Bitrate: ' + bitrate + ' kbps | Watermark: ' + state.hasWatermark);

        if (sizeMB <= CONFIG.quality.targetMaxMB) log('✅ WhatsApp: minimal re-compression expected');
        else if (sizeMB <= CONFIG.quality.absoluteMaxMB) log('⚠️ WhatsApp: light re-compression expected');
        else log('⚠️ WhatsApp: may re-compress significantly');
        ...
        sendNotification('🔥 Your video is crispy!', 'HD quality in ' + elapsed + 's. Tap to download.');
        haptic('success');
        showDone();
...
function showDone() {
    ...
    if(state.outputUrl&&els.donePreview){els.donePreview.src=state.outputUrl;
...
function shareToWhatsApp() {
    ...
    if(navigator.canShare){var file=new File([state.outputBlob],'crispy-status.mp4',{type:'video/mp4'});
    if(navigator.canShare({files:[file]})){navigator.share({files:[file]}).then(function(){showToast('Shared! 🚀','success');}).catch(function(){});return;}}
    downloadVideo();
}
...
function showTips() {
    ...
    QUALITY_TIPS.forEach(function(tip){var card=document.createElement('div');card.className='tip-card';
...
function setupInstallPrompt() {
    ...
    if(els.installYes)els.installYes.addEventListener('click',async function(){
        if(!state.installPrompt)return;await state.installPrompt.prompt();
        state.installPrompt=null;if(els.installPrompt)els.installPrompt.classList.add('hidden');
...
function truncateFilename(n,max){if(n.length<=max)return n;var ext=n.split('.').pop();return n.substring(0,max-ext.length-3)+'….'+ext;}
function cleanup(){
    if(state.objectUrl)URL.revokeObjectURL(state.objectUrl);if(state.outputUrl)URL.revokeObjectURL(state.outputUrl);
    ...
    if(els.fileInput)els.fileInput.value='';if(els.donePreview)els.donePreview.src='';if(els.trimVideo)els.trimVideo.src='';
}
...
function setupBackButton(){
    window.addEventListener('popstate',function(){
        ...
        if(c){if(c.id==='trim-screen'){if(els.trimVideo)els.trimVideo.pause();showScreen('home-screen');}
...
function bindEvents() {
    log('Binding events…');
    if(els.uploadBtn)els.uploadBtn.addEventListener('click',function(){log('Upload clicked');haptic('light');if(els.fileInput)els.fileInput.click();});
    ...
    if(els.playPreviewBtn)els.playPreviewBtn.addEventListener('click',function(){
        ...
        if(v.paused){v.currentTime=state.trimStart;v.muted=false;v.play();
        els.playPreviewBtn.textContent='⏸';els.playPreviewBtn.classList.add('hide');
        var sa=state.trimStart+CONFIG.maxDuration;
        var stop=function(){if(v.currentTime>=sa){v.pause();v.removeEventListener('timeupdate',stop);els.playPreviewBtn.textContent='▶';els.playPreviewBtn.classList.remove('hide');}};
    ...
    document.addEventListener('keydown',function(e){if(e.key==='Escape'&&els.errorModal&&!els.errorModal.classList.contains('hidden'))hideError();});
...
function preloadFFmpeg(){
    setTimeout(function(){if(!state.ffmpegReady){log('Preloading FFmpeg…');
    loadFFmpeg().then(function(){log('Preload ✅');}).catch(function(e){log('Preload failed: '+e.message);});}},3000);
}

/* ======================== INIT ======================== */
function init() {
    log('========================================');
    log('Crispy Status v9 — Maximum Quality');
    log('========================================');
    log('CRF: ' + CONFIG.quality.baseCRF + ' base (dynamic 14-20)');
    log('Preset: ' + CONFIG.quality.preset + ' (high quality per bit)');
    log('Scaler: Lanczos (razor sharp)');
    log('Bitrate: Dynamic 2500k-5000k by duration');
    log('x264: aq-mode=2 (adaptive quantization)');
    log('Fallback: 3-tier (watermark → no watermark → safe mode)');

    if(typeof WebAssembly==='undefined'){alert('Browser not supported. Use Chrome, Firefox, Safari, or Edge.');return;}

    initEls();
    try{bindEvents();}catch(e){logError('bindEvents failed',e);alert('App failed: '+e.message);return;}
